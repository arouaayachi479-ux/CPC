...
def extract_one_sheet(raw_bytes, sheet_name):
    ...
    for i, row in enumerate(ws.iter_rows(values_only=True), start=1):
        ...
        if i < 8:          # ← on ignore les lignes 1 à 7
            continue
        if len(row) < 31:
            continue
        libelle = row[1]
...
def build_styled_excel(df_result: pd.DataFrame) -> bytes:
    """
    Génère un fichier Excel aux couleurs LabelVie :
    - En-têtes : fond rouge #E30613, texte blanc, gras
    - Ligne de titre générale : fond vert #00A651
    - Lignes paires : fond gris très clair
...
missing = [c for c in [COL_CODE, COL_MAG, COL_SURFACE, COL_ALCOOL]
           if c not in df_check.columns]
if missing:
    st.error(
        f"❌ Colonnes introuvables : **{missing}**\n\n"
...
st.markdown('<div class="block-title">Aperçu — Check Alcool</div>',
            unsafe_allow_html=True)
st.dataframe(
    df_check[[COL_CODE, COL_MAG, COL_SURFACE, COL_ALCOOL]].head(10),
...
with mc1:
    st.markdown(
        ...
        f'<div class="metric-num">{len(magasins)}</div>'
        f'<div class="metric-label">Magasins Check</div></div>',
        unsafe_allow_html=True)
with mc2:
    st.markdown(
        f'<div class="metric-box">'
...
if unmatched:
    st.markdown(
        f"<div class='warn-banner'>⚠️ Magasins sans feuille CPC : "
...
if st.button("🔨  Construire la base consolidée"):
    ...
    for idx, (mag_name, sheet_name) in enumerate(matched):
        code_site, postes = extract_one_sheet(bytes_cpc, sheet_name)

        mask      = df_check[COL_MAG].astype(str).str.strip().str.lower() == mag_name.lower()
        ...
        surface  = row_check.get(COL_SURFACE) if not row_check.empty else None
        alcool   = row_check.get(COL_ALCOOL)  if not row_check.empty else None
    ...
    df_result = pd.DataFrame(all_rows, columns=[
        "Code Site", "Lib Magasin", "Poste", "Cumulé", "Surface", "Alcool"
    ])
